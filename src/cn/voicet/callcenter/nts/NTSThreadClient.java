...
public class NTSThreadClient {
		...
		private static Runnable createTask(final NTSThreadClient client,final String serverName,final String serverIp,final int port)
		{
	        return new Runnable()
	        {
	            public void run()
...
tryconnectagain:
		...
		private NTSMsg RecvMessage()
		{
	    	...
	    	{ 
		    	...
		    	if(len>0)
		    	{
			        ...
			        if(packLen>28)
			        {
			        msg.body =BytesConvert.BytesToString(databuff, "GBK",29,packLen-28);
			        }
			        else
			        	msg.body="";
					return msg;
		...
		public boolean SendMessage(String to,int action,int channel,int param,String body)
		{
			return SendMessage(sLocalName,to,action,channel,param,body);
		}
		public boolean SendMessage(String from,String to,int action,int channel,int param,String body)
		{
			int len=0;
			try {
		...
		{
			System.out.println("From:"+msg.from+",To:"+msg.to+",action:"+msg.action+",channel:"+msg.channel+",param:"+msg.param+",content:"+msg.body);
			switch(msg.action)
			{
				case 1:
				{
				...
				case 110:
				{
					System.out.println("帐号:"+sLocalName+"登录服务器:"+sNTSName+"成功");
				}
					break;
				case 111:
				{
					System.out.println("帐号:"+sLocalName+"登录服务器:"+sNTSName+"失败，原因["+msg.body+"]");
				}
				break;
				case 1000://心跳包
				{
